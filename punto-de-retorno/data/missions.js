...
(function (global) {
  ...
  VG.MISSION_CONTENT = [
    ...
    {
      phase: 2, severity: "high", source: "Arnold (en prácticas)", xp: 100,
      title: "¡Arnold ha roto servicios.conf!",
      text: "Arnold ha abierto servicios.conf en tu equipo y lo ha guardado a medio escribir. No ha hecho commit, así que la versión buena sigue a salvo en el último commit.",
    ...
    {
      phase: 2, severity: "critical", source: "Arnold (en prácticas)", xp: 100,
      title: "Un fichero que no debía estar",
      text: "Arnold ha hecho un git add . «para ayudarte» y se ha colado volcado.sql en el área de preparación: un volcado de la base de datos con datos personales de clientes. No puede llegar a un commit.",
    ...
    },
    {
      ...
      text: "Arnold ha hecho dos commits con el mensaje «wip» en tu rama main. Los cambios son útiles, pero esos commits ensucian el historial. Como no se han compartido, puedes deshacerlos… sin perder su contenido.",
    ...
    },
    {
      ...
      text: "Los cambios de Arnold están ahora en tu directorio de trabajo: un límite de conexiones para SSH y el registro del cortafuegos. Revísalos y guárdalos en un único commit con un mensaje que explique qué hacen.",
    ...
    {
      phase: 3, severity: "critical", source: "Sigourney (Blue Team)", xp: 150,
      title: "Conflicto a la vista",
      text: "Sigourney ha dejado su trabajo en la rama weaver/puertos. Ha tocado la misma línea de servicios.conf que tú: Git no puede decidir solo qué versión es la buena. Sigourney ha desinstalado vsftpd, así que la línea correcta es ftp=eliminado.",
      objective: "Fusiona weaver/puertos en main, resuelve el conflicto de servicios.conf y confirma la fusión.",
      hints: [
        "Tras git merge weaver/puertos, git status te dirá qué fichero está en conflicto («ambos modificados»).",
        "Ábrelo con nano: entre <<<<<<< HEAD y ======= está tu versión; entre ======= y >>>>>>> la de Sigourney. Deja el contenido final correcto y borra las tres líneas de marcas (Ctrl+K corta líneas).",
    ...
    {
      phase: 4, severity: "medium", source: "Sigourney (Blue Team)", xp: 100,
      title: "Novedades en el servidor",
      text: "Sigourney ha subido al servidor el informe de la auditoría. Tu repositorio no se entera solo: Git nunca descarga nada si no se lo pides.",
    ...
    },
    {
      ...
      text: "El escritorio remoto (3389/tcp) ya no se usa. Añade al cortafuegos una regla que lo bloquee y súbela al servidor. Ojo: Sigourney está trabajando ahora mismo en el mismo repositorio…",
      ...
      hints: [
        ...
        "git pull te pedirá que elijas cómo reconciliar ramas divergentes: --no-rebase (commit de fusión) o --rebase (reaplica tus commits encima de los de Sigourney).",
        "Escribe: git pull --rebase (o git pull --no-rebase) y después git push"
      ]
    },
    {
      phase: 4, severity: "critical", source: "Arnold (en prácticas)", xp: 150,
      title: "Revierte sin reescribir",
      text: "Arnold ha vuelto a abrir Telnet «para el switch del almacén» y lo ha subido al servidor. Como ese commit ya es público, no se debe borrar de la historia: hay que crear otro commit que lo deshaga.",
      objective: "Trae el commit de Arnold, deshazlo con git revert y sube la corrección a origin.",
      hints: [
        "Primero git pull para tener el commit de Arnold. Localízalo con git log --oneline y míralo con git show.",
    ...
    },
    {
      ...
      text: "Estás documentando el README cuando Sigourney te pide que compruebes algo en la versión publicada. Tus cambios no están listos para un commit, pero tampoco quieres perderlos.",
