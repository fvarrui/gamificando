...
> Reto digital de elaboración propia, hermano de [*Los intocables*](../los-intocables/)
...
*Control total* es un juego educativo que se ejecuta en el navegador. El
alumnado se pone en la piel de **Linda**, de sistemas, en el servidor de archivos
...
- **En el aula virtual (EVAGD/Moodle)**: subir la carpeta completa del reto **junto con la
  ...
  `control-total/index.html`. El reto usa CSS y scripts clásicos, sin módulos ES ni
...
1. **Portada (encargo)**: el correo de Sigourney Weaver, responsable de sistemas, con el
...
```bash
node .claude/skills/verificar-reto/scripts/run.mjs test-control-total
```
...
- **No se cambia de identidad**: no hay «Ejecutar como» ni forma de iniciar sesión como
  Dolph para comprobar en primera persona que la denegación le afecta. El efecto se razona
