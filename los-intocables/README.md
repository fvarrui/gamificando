...
*Los intocables* es un juego educativo que se ejecuta en el navegador. Quien juega
es **Linda Hamilton**, de sistemas de la empresa ficticia **TecnoAtlántica**: una consultora
...
- **En el aula virtual (EVAGD/Moodle)**: subir la carpeta completa del reto **junto con la
  ...
  `los-intocables/index.html`. El reto usa CSS y scripts clásicos, sin módulos ES ni
...
1. **Portada (encargo)**: el correo urgente de Sigourney Weaver, responsable de sistemas,
...
Las tareas llegan **de una en una**, como peticiones: casi todas de Sigourney (sistemas), algunas
de Michelle Yeoh (coordinadora del proyecto) y otras de Dirección. Al completar una, a los pocos
...
- **`sudo` está disponible, pero no eres `root`.** Linda pertenece a `sistemas`, `proyectos` y
...
| 7 | ACL-07 | Y su responsable, Michelle | `chown` sobre varios ficheros a la vez |
...
| 18 | ACL-18 | Jean-Claude solo necesita el acta | `setfacl -m u:vandamme:r`: ACL de usuario y mínimo privilegio |
| 19 | ACL-19 | Jean-Claude también necesita entrar | `setfacl -m u:vandamme:x` sobre el directorio: `x` atraviesa, `r` lista |
| 20 | ACL-20 | Se acabó el contrato | `setfacl -x u:lundgren`: retirar un acceso caducado |
...
Michelle, ni Linda, ni quien está administrando. Para arreglarlo hace falta `sudo`, y en un servidor
...
- **Mínimo privilegio como mecánica, no como consejo.** El escenario está construido para que
  siempre haya una respuesta cómoda y otra correcta: meter a Dolph en `proyectos` sería más
  fácil que ponerle una ACL de lectura; darle `r` a Jean-Claude sobre la carpeta sería más fácil que
...
- **Las decisiones tienen consecuencias.** Ampliar los permisos de «otros» sobre cualquier
  cosa, dar escritura a ventas sobre el presupuesto, dar a Jean-Claude lectura de toda la carpeta,
...
| | *Blindaje de la Red* | *Punto de retorno* | *Los intocables* |
|---|---|---|---|
...
| **Sorpresa** | ✅ | Las peticiones llegan sin avisar y cambian el plan: Ventas quiere el presupuesto, Dirección quiere que entre Jean-Claude, la consultora conserva una ACL olvidada | Reproduce cómo llegan de verdad las peticiones de acceso |
...
| **Avatar** | ❌ | — | No aporta: la identidad que importa es `hamilton` y sus grupos, y se consulta con `id` |
...
(ventas solo lee y Jean-Claude solo atraviesa, sin conceder nada de más), 🧬 *Pensando en el futuro*
...
- El docente circula y pregunta en lugar de resolver: «¿qué significa esa `s`?», «¿por qué
  Jean-Claude no llega al acta si tiene permiso de lectura sobre ella?», «¿esto se arregla con
...
- **Puesta en común**: ¿quién usó `chgrp` y quién `chown :proyectos`? ¿Octal o simbólico?
  ¿Alguien dio a Jean-Claude `r` en vez de `x`, y qué consecuencia tuvo? ¿Por qué no basta con
...
- **Ampliación**: repetir el bastionado en una máquina real o en una máquina virtual con los
  ...
  [permisos NTFS en Windows](../control-total/) y comparar los dos modelos.
...
```bash
node .claude/skills/verificar-reto/scripts/run.mjs test-los-intocables
```
...
- **No se cambia de usuario**: no hay `su` ni inicio de sesión como Michelle o Jean-Claude, así que el
