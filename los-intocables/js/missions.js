...
(function (global) {
  ...
  var LOGIC = {
    ...
    "ACL-07": {
      check: function () {
        return ["memoria.txt", "presupuesto.csv", "acta-direccion.txt"].every(function (f) {
          var n = node(DIR + "/" + f);
          return !!n && n.owner === "yeoh";
        });
      },
      solution: ["sudo chown yeoh memoria.txt presupuesto.csv acta-direccion.txt"]
    },
    "ACL-08": {
    ...
    "ACL-18": {
      check: function () { return acl(DIR + "/acta-direccion.txt", "user", "vandamme", "r"); },
      solution: ["sudo setfacl -m u:vandamme:r acta-direccion.txt"]
    },
    "ACL-19": {
      check: function () { return acl(DIR, "user", "vandamme", "x"); },
      react: function (ev) {
        if (ev.type === "setfacl" && ev.kind === "user" && ev.name === "vandamme" && /r/.test(ev.perms || "") &&
            ev.path && /proyectos$/.test(ev.path)) {
          S().game.addRisky("acl:dario-lectura",
            "le has dado a Jean-Claude permiso para listar toda la carpeta de proyectos, cuando solo necesitaba llegar al acta.");
        }
      },
      solution: ["sudo setfacl -m u:vandamme:x /srv/proyectos"]
    },
    "ACL-20": {
      check: function () { return noAcl(DIR + "/memoria.txt", "user", "lundgren"); },
      solution: ["sudo setfacl -x u:lundgren memoria.txt"]
    },
