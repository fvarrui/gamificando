...
*Usuarios y grupos en Linux* es un juego educativo que se ejecuta en el navegador. Quien
juega se pone en la piel de **Ana Betancor**, que administra el servidor `srv-datos` de la
...
- **En el aula virtual (EVAGD/Moodle)**: subir la carpeta completa del reto **junto con la
  ...
  `usuarios-y-grupos-en-linux/index.html`. El reto usa CSS y scripts clásicos, sin módulos
...
[`usuarios-y-grupos-en-windows/`](../usuarios-y-grupos-en-windows/), con cuentas locales,
...
1. **Portada (encargo)**: el correo de Nayra Suárez, responsable de sistemas, con las dos
...
- **Todo pasa por el modelo del sistema.** No hay respuestas «correctas» memorizadas: la
  ...
  `formacion`?, ¿está `elena` en `ventas`?, ¿tiene `dario` la cuenta bloqueada?) o el
...
- **Las decisiones arriesgadas cuestan, pero se permiten.** Un vigilante global observa
  ...
  final.
- **Los privilegios importan.** La sesión es de `ana`, no de `root`. Toda orden de
...
| 4 | USR-04 | La ficha de Carla | `id` / `groups`: *uid*, *gid* y todos los grupos |
...
| 7 | USR-07 | Alta de Elena | `useradd -m -s -c`: sin `-m` no hay directorio personal |
...
| 10 | USR-10 | Alta de Hugo, ya en su equipo | `useradd -G`: `-g` (principal) frente a `-G` (secundarios) |

### Fase 3 · Pertenencias

| # | Código | Tarea | Qué se practica |
|---|--------|-------|-----------------|
| 11 | USR-11 | Elena entra en ventas | **`usermod -aG`**: sin la `-a`, `-G` sustituye toda la lista |
| 12 | USR-12 | Compruébalo | `id`: comprobar siempre después de tocar grupos |
| 13 | USR-13 | Hugo, al plan de formación | `gpasswd -a usuario grupo` (ojo al orden de los argumentos) |
| 14 | USR-14 | Darío sale del proyecto | `gpasswd -d` para quitar una pertenencia |
...
| # | Código | Tarea | Qué se practica |
|---|--------|-------|-----------------|
| 16 | USR-16 | El estado de la cuenta de Darío | `passwd -S`: `P`, `L` o `NP` |
...
- **La ficha describe el problema, nunca la solución.** Los objetivos están escritos en
  términos del resultado («añade a `elena` al grupo secundario `ventas` sin sacarla de
...
- **Los mensajes de error orientan sin dar la respuesta.** «El grupo *inventado* no existe»,
  «el usuario *elena* ya existe», «no se puede eliminar el grupo primario del usuario
  *ana*»: cada fallo dice qué está mal, no qué escribir.

### Evolución del diseño

| Se probó | Qué pasó | Cómo quedó |
|---|---|---|
| Un panel lateral que dibujase usuarios y grupos, al estilo del grafo de *Versionando con Git* | Quitaba sentido al contenido: si el panel muestra las pertenencias, nadie escribe `id` ni `getent group`, que es justo lo que hay que aprender | Sin panel visual. La verificación se hace con órdenes, y hay tareas dedicadas a ello |
...
| Empezar directamente por el alta de Elena | Sin ver antes el sistema, las tareas se resolvían copiando la orden sin entender el contexto | Fase 1 entera de solo lectura: quién hay, qué grupos hay, cómo se consulta |
| Una única tarea «da de baja a Darío» | Mezclaba dos decisiones distintas (bloquear la contraseña y quitar el intérprete) y ocultaba la idea central | Tres tareas separadas: ver el estado, bloquear, confirmar; y en la fase 5, quitar la shell como segunda barrera |
...
**más corto y más monotemático** que *Versionando con Git*: 25 tareas de una sola familia
...
| **Sorpresa** | ✅ | Las peticiones llegan de tres orígenes distintos (Sistemas, Personas y Talento, Proyectos) y alguna corrige lo anterior («se nos pasó: Elena necesita ventas») | Reproduce cómo llega el trabajo de verdad: a trozos y con rectificaciones |
...
| **Avatar** | ❌ | — | No aporta: la identidad del juego es la cuenta `ana`, que además condiciona qué se puede hacer sin `sudo` |
...
- Sugerencia de reparto: fases 1-3 en una sesión y 4-5 en otra, usando el **selector de
  fase** de la portada.
- El docente circula y pregunta en lugar de resolver: «¿en qué grupos está ahora Elena?»,
  «¿qué crees que pasará si quitas la `-a`?», «¿por qué esta cuenta se borra y la de Darío
  no?».
...
- **Puesta en común** a partir del informe final, comparando la columna «Cómo se resolvió»:
  ...
  - ¿Por qué la cuenta de Darío se bloquea y la de `visitas` se elimina? ¿Qué habría pasado
    con los ficheros de Darío si la hubiéramos borrado?
...
```bash
node .claude/skills/verificar-reto/scripts/run.mjs test-usuarios-linux
```
...
- que el **modelo de usuarios funciona de verdad**: Elena conserva su grupo principal y el
...
- que el **error clásico queda anotado**: empezando en la fase 3 y escribiendo
  `usermod -G ventas elena`, aparece el aviso de decisión arriesgada.
