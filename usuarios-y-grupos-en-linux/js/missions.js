...
(function (global) {
  ...
  var LOGIC = {
    ...
    "USR-04": {
      check: function (ev) { return !!ev && (ev.type === "id" || ev.type === "groups") && ev.user === "carla"; },
      solution: ["id carla"]
    },
    "USR-05": {
    ...
    "USR-07": {
      check: function () {
        var u = user("elena");
        return !!u && u.shell === "/bin/bash" && /Elena/i.test(u.comment) && !!node("/home/elena");
      },
      solution: ["sudo useradd -m -s /bin/bash -c \"Elena Quintana\" elena"]
    },
    "USR-08": {
    ...
    "USR-09": {
      check: function (ev) { return !!ev && ev.type === "passwd" && ev.name === "elena" && ev.set; },
      solution: ["sudo passwd elena"]
    },
    "USR-10": {
      check: function () {
        var u = user("hugo");
        return !!u && /Hugo/i.test(u.comment) && !!node("/home/hugo") && inGroup("hugo", "proyectos");
      },
      solution: ["sudo useradd -m -c \"Hugo Santana\" -G proyectos hugo"]
    },

    /* ---------------- FASE 3 · PERTENENCIAS ---------------- */
    "USR-11": {
      check: function () { return inGroup("elena", "ventas"); },
      solution: ["sudo usermod -aG ventas elena"]
    },
    "USR-12": {
      check: function (ev) { return !!ev && (ev.type === "id" || ev.type === "groups") && ev.user === "elena"; },
      solution: ["id elena"]
    },
    "USR-13": {
      check: function () { return inGroup("hugo", "formacion"); },
      solution: ["sudo gpasswd -a hugo formacion"]
    },
    "USR-14": {
      check: function () { return !!user("dario") && !inGroup("dario", "proyectos"); },
      solution: ["sudo gpasswd -d dario proyectos"]
    },
    "USR-15": {
    ...
    "USR-16": {
      check: function (ev) { return !!ev && ev.type === "passwd" && ev.name === "dario" && ev.status; },
      solution: ["sudo passwd -S dario"]
    },
    "USR-17": {
      check: function () {
        var u = user("dario");
      ...
      solution: ["sudo usermod -L dario"]
    },
    "USR-18": {
      check: function (ev) {
        return !!ev && ev.type === "passwd" && ev.name === "dario" && ev.status && ev.locked;
      },
      solution: ["sudo passwd -S dario"]
    },
    "USR-19": {
    ...
    "USR-23": {
      ...
      solution: ["sudo usermod -s /usr/sbin/nologin dario"]
    },
    "USR-24": {
    ...
    "USR-25": {
      check: function () {
        var n = node("/home/ana/cuentas.txt");
  ...
  UG.watch = function (S, ev) {
    ...
    if (ev.type === "userdel" && ev.name === "dario") {
      S.game.addRisky("userdel:dario",
        "has eliminado la cuenta de Darío en vez de bloquearla: sus ficheros se quedan sin dueño y no hay vuelta atrás.");
    }
    if (ev.type === "userdel" && ["ana", "bruno", "carla", "root"].indexOf(ev.name) !== -1) {
