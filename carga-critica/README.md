...
> y de [*Punto de retorno*](../punto-de-retorno/).
...
*Carga crítica* es un juego educativo que se ejecuta en el navegador. El
alumnado se pone en la piel de **Linda Hamilton**, del equipo de sistemas de la empresa
...
- **En el aula virtual (EVAGD/Moodle)**: subir la carpeta completa del reto **junto con la
  ...
  `carga-critica/index.html`. El reto usa CSS y scripts clásicos, sin módulos ES
...
1. **Portada (encargo)**: el correo de Sigourney Weaver, responsable de sistemas, explicando la
...
el lateral. Vienen firmadas: unas las manda Sigourney (sistemas) y otras Michelle Yeoh
...
| Decisión | Motivo |
|---|---|
| **Sin panel visual de contenedores**, a diferencia del grafo de *Punto de retorno* | Ahí el panel era imprescindible porque el modelo de Git es invisible; aquí la vista canónica ya existe y es `docker ps`. Dibujarla aparte enseñaría a mirar el dibujo en vez de a leer la salida real |
...
| | *Blindaje de la Red* | *Punto de retorno* | *Carga crítica* |
|---|---|---|---|
...
| Personajes | El SOC | Arnold y Sigourney | Sigourney (sistemas) y Michelle (desarrollo) |
...
| **Avatar** | ❌ | — | No aporta; la identidad es la de Linda, con su grupo `docker` y sus permisos |
...
| `data/config.js` | El escenario: `srv-docker`, la usuaria `hamilton` y su grupo `docker`, el registro de imágenes, `~/encargo.txt`, `~/intranet` (con el `Dockerfile`) y `~/pila` (con el `compose.yaml`) |
...
```bash
node .claude/skills/verificar-reto/scripts/run.mjs test-carga-critica
```
