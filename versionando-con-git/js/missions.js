...
(function (global) {
  ...
  function ikerWipCommits() {
    ...
    bcastFrom("iker", "He hecho dos commits «wip» en tu main con cosas que me pediste (el límite de SSH y el registro).\nNo he subido nada al servidor. ¿Los dejo así?");
  }
  function nayraBranch() {
    ...
    g.branches["nayra/puertos"] = c;
    state.mem.nayraTip = c;
    bcastFrom("nayra", "Os dejo mi trabajo en la rama nayra/puertos: he desinstalado vsftpd del servidor, así que en servicios.conf\nel FTP ya no está «desactivado», está «eliminado». Fusionadla cuando podáis.");
  }
  function nayraPushAudit() {
    ...
    bcastFrom("nayra", "He subido al servidor el resumen del informe de auditoría. Actualizaos antes de seguir tocando los scripts.");
  }
  function nayraPushSilent() {
    var tip = state.server.refs.main;
    if (!tip) { return; }
    var t = copy(VG.treeOf(tip));
    t["auditoria.md"] = (t["auditoria.md"] || "# Informe de auditoría\n") + "\n- Revisión de la semana 38: firmada por Nayra.\n";
  ...
  function ikerPushTelnet() {
    ...
    state.server.refs.main = c;
    state.mem.ikerTip = c;
    bcastFrom("iker", "He reabierto Telnet porque el switch del almacén solo habla ese protocolo, y ya lo he subido al servidor.\n¿Está bien así? Me han dicho que Telnet era malo, pero funcionaba…");
  }
  ...
  var LOGIC = {
    "GIT-01": {
      ...
      solution: ["git config --global user.name \"Ana Pérez\"", "git config --global user.email \"ana.perez@tecnoatlantica.local\""]
    },
    "GIT-02": {
    ...
    "GIT-11": {
      onActivate: function () {
        ...
        bcastFrom("iker", "¡Perdona! He abierto tu servicios.conf para mirar una cosa y le he dado a guardar sin querer. No he hecho commit, lo juro.");
      },
      check: function () {
        var head = VG.headTree();
    ...
    "GIT-12": {
      onActivate: function () {
        ...
        bcastFrom("iker", "He hecho un git add . en tu terminal para adelantarte trabajo. Creo que se ha colado un volcado de la base de datos, ¿eso es malo?");
      },
      check: function () {
        var head = VG.headTree();
    ...
    "GIT-14": {
      ...
      react: function (ev) {
        if (ev.type === "reset" && ev.mode === "hard" && VG.headHash() === state.mem.wipBase) {
          term.sys("warn", "⚠ Con --hard has descartado también los cambios de Iker. Tranquilidad: los commits siguen en el reflog. " +
    ...
    "GIT-26": {
      ...
      react: function (ev) {
        if (ev.type === "push" && ev.rejected) {
          term.sys("info", "ℹ El servidor ha rechazado el push: tiene commits que tú no tienes (Nayra acaba de subir algo). Intégralos primero con git pull.");
        }
      },
    ...
    "GIT-27": {
      ...
      react: function (ev) {
        if (ev.type === "reset" && state.mem.ikerTip && VG.headHash() && !VG.isAncestor(state.mem.ikerTip, VG.headHash()) &&
          ...
          term.sys("info", "ℹ Has sacado el commit de Iker de tu rama con reset, pero sigue en el servidor: al hacer push serías rechazado " +
