/* Recorrido completo de «Usuarios y grupos en Linux»: resuelve las 25
...
const t = checker();
const b = await launch(fileUrl('usuarios-y-grupos-en-linux/index.html'), { port: 9406 });
const g = game(b);
const check = t.check;
...
const PASOS = [
  ...
  ['USR-07', ['sudo useradd -m -s /bin/bash -c "Elena Quintana" elena']],
  ['USR-08', ['ls -l /home']],
  ['USR-09', ['sudo passwd elena']],
  ['USR-10', ['sudo useradd -m -c "Hugo Santana" -G proyectos hugo']],
  ['USR-11', ['sudo usermod -aG ventas elena']],
  ['USR-12', ['id elena']],
  ['USR-13', ['sudo gpasswd -a hugo formacion']],
  ['USR-14', ['sudo gpasswd -d dario proyectos']],
  ['USR-15', ['getent group proyectos']],
  ['USR-16', ['sudo passwd -S dario']],
  ['USR-17', ['sudo usermod -L dario']],
  ['USR-18', ['sudo passwd -S dario']],
  ['USR-19', ['sudo userdel -r visitas']],
  ['USR-20', ['ls -l /home']],
  ['USR-21', ['sudo useradd -r -s /usr/sbin/nologin svc-backup']],
  ['USR-22', ['getent passwd svc-backup']],
  ['USR-23', ['sudo usermod -s /usr/sbin/nologin dario']],
...
try {
  ...
  const elena = await g.run('id elena');
  check(elena.includes('ventas') && elena.includes('elena'), 'Elena conserva su grupo principal y el secundario', elena);
  ...
  const dario = await g.run('sudo passwd -S dario');
  check(/dario L/.test(dario), 'la cuenta de la baja queda bloqueada, no borrada', dario);
  ...
  check(home.includes('elena') && home.includes('hugo') && !home.includes('visitas'),
    'los directorios personales reflejan las altas y la baja', home);
  const dup = await g.run('sudo useradd -m elena');
  check(dup.includes('ya existe'), 'no se puede crear dos veces la misma cuenta', dup);
  const malGrupo = await g.run('sudo usermod -aG inventado hugo');
  ...
  await g.type('sudo usermod -G ventas elena');
