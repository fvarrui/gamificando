/* Recorrido completo de «Punto de retorno»: las 30 misiones
...
const SOLUCIONES = [
  ['GIT-01', ['git config --global user.name "Linda Pérez"', 'git config --global user.email "hamilton.perez@tecnoatlantica.local"']],
  ...
  ['GIT-22', ['git merge weaver/puertos', '__CONFLICTO__']],
...
const FASES = [
  ...
  [4, 'GIT-23', async (g, check) => {
    ...
    check((await g.run('cat servicios.conf')).includes('ftp=eliminado'), 'fase 5: el conflicto de Sigourney quedó resuelto');
    check(!(await g.run('git branch')).includes('endurecer-ssh'), 'fase 5: la rama fusionada ya se borró');
  }]
];

const t = checker();
const b = await launch(fileUrl('punto-de-retorno/index.html'), { port: 9392 });
const g = game(b);
const check = t.check;
...
try {
  ...
  check((await b.eval(`document.title`)).includes('Punto de retorno'), 'el título de la página es correcto');
  ...
  for (const [code, cmds] of SOLUCIONES) {
    ...
    for (const cmd of cmds) {
      if (cmd === '__IGNORADO__') {
        ...
        out = await g.term();
        check(out.includes('CONFLICTO (contenido): Conflicto de fusión en servicios.conf'), 'la fusión con weaver/puertos da conflicto');
        ...
        await g.run('git merge weaver/puertos');
        await g.type('nano servicios.conf');
        await g.nanoOpen();
        const conflicted = await g.nanoValue();
        check(conflicted.includes('<<<<<<< HEAD') && conflicted.includes('>>>>>>> weaver/puertos'),
