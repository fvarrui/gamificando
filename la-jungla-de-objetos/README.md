# La jungla de objetos

> Simulador gamificado de la consola de Windows, donde todo son objetos.
> Reto digital de elaboración propia, hermano de [*Terminal implacable*](../terminal-implacable/)
...
*La jungla de objetos* es un juego educativo que se ejecuta en el navegador. El
...
una en una** y las firman dos personas del equipo: **Arnold Schwarzenegger** (administración de
sistemas), que va soltando el vocabulario, y **Sigourney Weaver** (soporte), que pide los datos
...
- **En el aula virtual (EVAGD/Moodle)**: subir la carpeta completa del reto **junto con la
  ...
  `la-jungla-de-objetos/index.html`. El reto usa CSS y scripts clásicos, sin módulos
...
1. **Portada (bienvenida)**: mensaje de Arnold Schwarzenegger presentando el equipo de prácticas,
...
2. **Juego**: la consola ocupa casi toda la pantalla, con el aspecto y el *prompt* de Windows
   PowerShell (`PS C:\Users\hamilton>`). Arriba, una barra con el progreso por fases, el
...
*Punto de retorno*) resultaba desconcertante en un primer día, cuando aún no se sabe
...
| **Avatar** | ❌ | — | No aporta: la identidad ya está en el *prompt* (`hamilton@WS-PRACTICAS`) |
...
```bash
node .claude/skills/verificar-reto/scripts/run.mjs test-jungla-de-objetos
```
