...
*Servicios en Linux* es un juego educativo que se ejecuta en el navegador. El alumnado asume
el papel de **Ana**, la persona que está de guardia esa noche en la empresa ficticia
...
- **En el aula virtual (EVAGD/Moodle)**: subir la carpeta completa del reto **junto con la
  ...
  `servicios-en-linux/index.html`. El reto usa CSS y scripts clásicos, sin módulos ES ni
...
firmada por quien la pide —el **centro de guardia**, **Nayra Suárez** (compañera de sistemas)
...
| **Avatar** | ❌ | — | No aporta; la identidad ya la da el papel de Ana y el `prompt` `ana@srv-web` |
...
```bash
node .claude/skills/verificar-reto/scripts/run.mjs test-servicios-linux
```
