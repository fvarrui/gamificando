...
> [*Usuarios y grupos en Linux*](../usuarios-y-grupos-en-linux/): la misma semana de altas y
...
*Usuarios y grupos en Windows* es un juego educativo que se ejecuta en el navegador. Quien
juega es **Ana Betancor**, del equipo de sistemas de la empresa ficticia **TecnoAtlántica**, y
...
- **En el aula virtual (EVAGD/Moodle)**: subir la carpeta completa del reto **junto con la
  ...
  `usuarios-y-grupos-en-windows/index.html`. El reto usa CSS y scripts clásicos, sin módulos ES
...
El reto es el **espejo en Windows** de *Usuarios y grupos en Linux*: mismo escenario, mismas
...
1. **Portada (encargo)**: el mensaje de Nayra Suárez, responsable de sistemas, con el encargo de
...
Las tareas llegan **de una en una**, como peticiones con remitente: Nayra (sistemas), Personas y
Talento, o Carla Ojeda (proyectos). Al completar una, a los pocos segundos entra la siguiente con
...
- las tareas que cambian algo (crear, añadir, quitar, deshabilitar, eliminar) se validan mirando
  el modelo: existe el grupo `Formacion`, `hugo` está en `Proyectos`, `dario` ya no tiene la
...
- **Decisiones arriesgadas**: las acciones dañinas **se permiten**. Eliminar la cuenta de Darío
...
- **Consola con privilegios**: la sesión está elevada porque `ana` pertenece a Administradores.
...
| 7 | CTA-07 | Alta de Elena | `New-LocalUser -NoPassword -Description` (y el `SecureString` de `-Password`) |
| 8 | CTA-08 | Comprueba la cuenta | `Get-LocalUser <cuenta>`, `Format-List` |
| 9 | CTA-09 | Alta de Hugo | Repetir el alta con otro perfil y otra descripción |
| 10 | CTA-10 | Y a su equipo | `Add-LocalGroupMember -Group Proyectos -Member hugo` |

### Fase 3 · Pertenencias

| # | Tarea | Título | Qué se practica |
|---|-------|--------|-----------------|
| 11 | CTA-11 | Elena, al equipo comercial | Añadir a un grupo sin salir de los demás |
| 12 | CTA-12 | Compruébalo | Verificar la pertenencia justo después de tocarla |
| 13 | CTA-13 | Hugo, al plan de formación | Pertenencia múltiple: los permisos se suman |
| 14 | CTA-14 | Darío sale del proyecto | `Remove-LocalGroupMember` sin tocar la cuenta |
...
| # | Tarea | Título | Qué se practica |
|---|-------|--------|-----------------|
| 16 | CTA-16 | La ficha de Darío | Mirar el estado antes de decidir: `Enabled` en `True` |
...
| 23 | CTA-23 | Corrige la ficha de Elena | `Set-LocalUser -Description`: modificar sin recrear |
...
2. **Añadir a un grupo no quita de los demás.** Quien viene de Linux arrastra la cicatriz de
   ...
   juego lo dice en una pista y lo demuestra dejando a Hugo en `Usuarios`, `Proyectos` y
   `Formacion` a la vez.
...
C:\Users\ana>` y las salidas tabulares de PowerShell. Los nombres de cuentas y grupos del sistema
van **sin tildes** (`dario`, `Formacion`, `Direccion`), como se nombran en la práctica para
...
- **Las decisiones dañinas se permiten, no se bloquean.** Se puede eliminar la cuenta de Darío en
...
| | Retos anteriores | *Usuarios y grupos en Windows* |
|---|---|---|
...
| Paralelo | — | Espejo exacto de *Usuarios y grupos en Linux*, para comparar dos formas de hacer lo mismo |
...
- El profesorado circula y pregunta en lugar de resolver: «¿qué pasa con sus permisos si eliminas
  esa cuenta?», «¿se ha salido Hugo de `Usuarios` al meterlo en `Proyectos`?», «¿por qué esta
...
- **Puesta en común**: ¿quién eliminó la cuenta de Darío y qué pasó? ¿Qué diferencia hay entre
...
```bash
node .claude/skills/verificar-reto/scripts/run.mjs test-usuarios-windows
```
...
- la **partida completa**: las 25 tareas resueltas una a una con los comandos que propone cada
  ficha;
- que **el modelo de cuentas funciona de verdad**: Elena queda habilitada y con la descripción
  corregida, `Ventas` tiene a sus dos miembros, la baja de Darío queda **deshabilitada y no
...
| [`../usuarios-y-grupos-en-linux/`](../usuarios-y-grupos-en-linux/) | El mismo escenario resuelto en Linux |
