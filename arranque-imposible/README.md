...
> Reto digital de elaboración propia, hermano de [*Asalto al puerto 80*](../asalto-al-puerto-80/),
...
*Arranque imposible* es un juego educativo que se ejecuta en el navegador. El alumnado hace
la **guardia de mañana** de TecnoAtlántica: son las 7:40 del 20 de octubre, la aplicación de
nóminas no arranca y **nadie puede fichar**. Con la cuenta `hamilton` (del grupo Administradores),
...
- **En el aula virtual (EVAGD/Moodle)**: subir la carpeta completa del reto **junto con la
  ...
  `arranque-imposible/index.html`. El reto usa CSS y scripts clásicos, sin módulos ES ni
...
El reto es el **gemelo Windows** de *Asalto al puerto 80*: misma duración, mismas fases y misma
...
de guardia, Sigourney Weaver de Sistemas o el departamento de Personas y Talento), sus síntomas y
...
### Comparación con *Asalto al puerto 80*

| | *Asalto al puerto 80* | *Arranque imposible* |
|---|---|---|
...
| **Avatar** | ❌ | — | No aporta: la identidad es la cuenta `hamilton` y sus privilegios, que sí importan |
...
- Si ya se jugó *Asalto al puerto 80*, recordar qué hacía `systemctl enable`/`disable` y
...
| `data/config.js` | El escenario: el equipo `SRV-APP`, la cuenta `hamilton`, la fecha y hora del incidente, los **6 servicios** con su estado, tipo de inicio, PID y registro, y los archivos del disco |
...
```bash
node .claude/skills/verificar-reto/scripts/run.mjs test-arranque-imposible
```
