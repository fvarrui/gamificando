...
*Primeros pasos en Linux* es un juego educativo que se ejecuta en el navegador y está pensado
...
una carpeta personal, `/home/practicas`, que hay que aprender a recorrer.

Las tareas llegan **de una en una**, firmadas por dos personas del equipo: **Nayra Suárez**
(administración de sistemas), que enseña lo básico, e **Iker Alonso** (soporte), que aparece
...
- **En el aula virtual (EVAGD/Moodle)**: subir la carpeta completa del reto **junto con la
  ...
  `primeros-pasos-en-linux/index.html`. El reto usa CSS y scripts clásicos, sin módulos ES ni
...
Git*](../versionando-con-git/) y los de permisos, usuarios o servicios), que dan por sabido
moverse por una terminal. Tiene un gemelo para el otro mundo,
[*Primeros pasos en PowerShell*](../primeros-pasos-en-powershell/), con las mismas 27 tareas
...
1. **Portada**: mensaje de bienvenida de Nayra, explicación breve del juego, **selector de
   fase inicial**, datos del equipo (`srv-practicas`, usuario `practicas`, `/home/practicas`,
...
- **Los permisos son de verdad.** El sistema simulado tiene usuarios, grupos y modos POSIX:
  `/home/nayra` es `750` y no deja entrar, `/root` es `700` y responde «Permiso denegado», y
...
[*Versionando con Git*](../versionando-con-git/), al comprobar en el aula un problema muy
...
|---|---|---|---|
| **Misiones** | ✅ | 27 tareas encadenadas (LNX-01…LNX-27) agrupadas en 5 fases, firmadas por Nayra (sistemas) o por Iker (soporte) | Se habla de «tareas del equipo», nunca de ejercicios: cada encargo tiene remitente y motivo, así que no se hace `wc -l` «para practicar», sino para el parte de incidencias |
...
| **Avatar** | ❌ | — | No aporta: la identidad ya está en el indicador (`practicas@srv-practicas`) y se descubre con `whoami` e `id` |
...
- **Explorer**: hay mucho más sistema del que piden las tareas. `/srv/compartido` tiene una
  chuleta de consola escrita por Nayra, `/etc/os-release` y `/etc/hostname` se pueden leer,
  `/home/nayra` y `/root` están cerrados a propósito, existen `chmod`, `stat`, `getfacl` y
...
- **Ampliación**: repetir lo mismo en una máquina real o en una máquina virtual con Ubuntu, y
  comprobar que las órdenes se comportan igual; o continuar con *Primeros pasos en PowerShell*
...
```bash
node .claude/skills/verificar-reto/scripts/run.mjs test-linux-basico
```
...
- **Los permisos existen pero no se enseñan**: se choca con ellos (`/root`, `/home/nayra`) y
